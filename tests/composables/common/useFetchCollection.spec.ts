...
describe("useFetchCollection:", () => {
  ...
  it("creates useFetchCollection composable", () => {
    const result = composable(new URL("https://block-explorer-api.testnets.micro.dev"));
  ...
  describe("estimate", () => {
    ...
    beforeEach(() => {
      fc = composable(new URL("https://block-explorer-api.testnets.micro.dev"));
    });
    ...
    it("maps and sets corresponding data when item mapper is specified and request is completed", async () => {
      fc = composable(new URL("https://block-explorer-api.testnets.micro.dev"), (item) => `mapped ${item}`);
      ($fetch as unknown as SpyInstance).mockResolvedValue({
        items: ["item1", "item2"],
        meta: {},
      });
    ...
    it("sets toDate to query string when param specified", async () => {
      ...
      expect($fetch).toHaveBeenCalledWith(
        "https://block-explorer-api.testnets.micro.dev/?pageSize=10&page=1&toDate=2023-05-01T10%3A00%3A00.000Z"
      );
    });
  });
});
