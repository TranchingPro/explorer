...
describe("useFetch:", () => {
  ...
  describe("estimate", () => {
    ...
    it("uses provided url", async () => {
      fc = composable(() => new URL("https://block-explorer-api.testnets.micro.dev/?a=b"));

      await fc.fetch();

      expect($fetch).toHaveBeenCalledWith("https://block-explorer-api.testnets.micro.dev/?a=b");
    });
  });
});
