...
vi.mock("../../src/configs/local.config", () => {
  return {
    default: {
      networks: [CALIBRATION_BETA_NETWORK, CALIBRATION_NETWORK],
    },
  };
});
...
vi.mock("../../src/configs/staging.config", () => {
  return {
    default: {
      networks: [CALIBRATION_BETA_NETWORK, { ...CALIBRATION_NETWORK, published: false }],
    },
  };
});
...
describe("useEnvironmentConfig:", () => {
  ...
  describe("loadEnvironmentConfig", () => {
    it("sets networks data to config", async () => {
      ...
      expect(networks.value).toEqual([CALIBRATION_BETA_NETWORK, CALIBRATION_NETWORK]);
    });
  ...
  describe("networks", () => {
    ...
    it("returns only published network configs", async () => {
      ...
      expect(networks.value).toEqual([CALIBRATION_BETA_NETWORK]);
    });
  });
});
