...
import type { Provider } from "@zkamoeba/micro-web3";
...
export const CALIBRATION_NETWORK: NetworkConfig = {
  name: "calibration",
  verificationApiUrl: "",
...
export const CALIBRATION_BETA_NETWORK: NetworkConfig = {
  name: "calibration-beta",
  verificationApiUrl: "",
  apiUrl: "https://block-explorer-api.mock.micro.dev",
  icon: "",
  l2ChainId: 270,
  rpcUrl: "",
  l2NetworkName: "Calibration Beta",
  l2WalletUrl: "",
  l1ExplorerUrl: "",
  maintenance: false,
  published: true,
  hostnames: ["https://calibration-beta.staging-scan-v2.micro.dev/"],
};
  ...
  const mockWallet = vi.spyOn(composablesFactory, "useWallet").mockReturnValue({
    ...composablesFactory.useWallet({
      currentNetwork: computed(() => ({
        chainName: CALIBRATION_NETWORK.name,
        explorerUrl: CALIBRATION_NETWORK.l1ExplorerUrl!,
        l1ChainId: 5,
        l2ChainId: CALIBRATION_NETWORK.l2ChainId,
        rpcUrl: CALIBRATION_NETWORK.rpcUrl,
      })),
...
export const useContextMock = (params: any = {}) => {
  const mockContextConfig = vi.spyOn(useContext, "default").mockReturnValue({
    ...
    currentNetwork: computed(() => CALIBRATION_NETWORK),
    identifyNetwork: () => undefined,
    isReady: computed(() => true),
    networks: computed(() => [CALIBRATION_NETWORK]),
    ...params,
  });
